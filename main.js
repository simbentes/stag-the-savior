...
function redimensiona() {
  if (
    parseInt(t.style.height) > window.innerHeight ||
    parseInt(t.style.width) < window.innerWidth
  ) {
    t.style.height = `${window.innerHeight}px`;
    t.style.width = `${window.innerHeight * 1.6}px`;
  }
...
let delta;
const audioCol = new Audio('sounds/colLenhador.mp3');
let tparado = false;
var tLargura = parseInt(t.style.width);
var tAltura = parseInt(t.style.height);
const movimento_vertical = [];
const mov_vertical_sentido = [];
let tecla_a = false;
let tecla_w = false;
let tecla_d = false;
let tecla_s = false;
let tecla_enter = false;
let direcao = 'drt';
let seg;
let min;
let tempoTotal;
let pontos;
const audioMenus = new Audio('sounds/tecla1.mp3');
const audioClicou = new Audio('sounds/tecla2.mp3');
const audioErro = new Audio('sounds/errocompra.mp3');
let hover = 0;

const musica = new Audio('sounds/vacalourademo1.m4a');
musica.loop = true;

const musicaFim = new Audio('sounds/doors.mp3');
musica.loop = true;
...
const creditos1 = new Image();
creditos1.src = 'assets/logoua.png';

const fundoMenu = new Image();
fundoMenu.src = 'assets/background-menu.png';

const melhorP = new Image();
melhorP.src = 'assets/melhor.png';

const fimDoJogo = new Image();
fimDoJogo.src = 'assets/fimdoJogo.png';

const fundo = new Image();
fundo.src = 'assets/jogoback.png';

const fundoPausa = new Image();
fundoPausa.src = 'assets/fundopausa.png';

const fundoLoja = new Image();
fundoLoja.src = 'assets/fundoloja.png';

const botoesPausa = new Image();
botoesPausa.src = 'assets/botoes_pausa.png';

const floresta = new Image();
floresta.src = 'assets/floresta.png';

const arvoreMae = new Image();
arvoreMae.src = 'assets/arvore_mae.png';

const zonaEntrada = new Image();
zonaEntrada.src = 'assets/zona_entrada.png';

const avisoDisparo = new Image();
avisoDisparo.src = 'assets/aviso-disparo.png';

const barraPontos = new Image();
barraPontos.src = 'assets/barratempopontos.png';

const moedaIcon = new Image();
moedaIcon.src = 'assets/moeda_icon.png';

const instruimg = new Image();
instruimg.src = 'assets/instrucoes.png';

const sobreimg = new Image();
sobreimg.src = 'assets/sobre.png';
...
let melhorPontos = localStorage.getItem('melhorPontos');

if (localStorage.getItem('carteira') === null)
  localStorage.setItem('carteira', 0);

let carteira = localStorage.getItem('carteira');

if (localStorage.getItem('velocidade') === null)
  localStorage.setItem('velocidade', 1);

let upgradeVel = localStorage.getItem('velocidade');

if (localStorage.getItem('tempo') === null) localStorage.setItem('tempo', 1);

let upgradeTempo = localStorage.getItem('tempo');
...
jogador.img.src = 'assets/stag.png';
...
const lenhador = {
  ...
  posicionar() {
    ...
    let n = 0;
    while (n < this.num) {
      const pos_x_temp = nAleatorio(1280, this.areax);
      const pos_y_temp = nAleatorio(124, 676);
  ...
  mover() {
    for (let i = 0; i < this.num; i++) {
      animacaoSprite(
        this.img[this.cor[i]],
        this.largura * this.spriteX,
        this.altura * this.spriteY[i],
        this.largura,
        this.altura,
        this.x[i],
        this.y[i],
...
lenhador.img[0].src = 'assets/lenhador.png';
lenhador.img[1].src = 'assets/lenhador1.png';
lenhador.img[2].src = 'assets/lenhador2.png';
...
const familiar = {
  ...
  posicionar() {
    ...
    let n = 0;
    while (n < this.num) {
      const pos_x_temp = nAleatorio(15, 140);
      const pos_y_temp = nAleatorio(jogo.height, 1600);
      let sobreposicao = false;
      for (let item = 0; item < this.num; item++) {
        if (
          pos_x_temp >= this.x[item] - this.largura &&
  ...
  mover() {
    for (let i = 0; i < this.num; i++) {
      ...
      } else if (this.tipo.length < this.num) {
        this.tipo.push(1);
      }
      animacaoSprite(
        this.img[this.tipo[i]],
        64 * this.spriteX,
        0,
        this.largura,
        this.altura,
...
familiar.img[0].src = 'assets/familiar.png';
familiar.img[1].src = 'assets/familiar2.png';

var moeda = {
  img: new Image(),
  audio: new Audio('sounds/moeda.mp3'),
  x: -36,
  y: -40,
  largura: 36,
  altura: 40,
...
moeda.img.src = 'assets/moeda.png';

var loja = {
  botaoSair: new Image(),
  item: [new Image(), new Image(), new Image(), new Image()],
  estado: [
    localStorage.getItem('item0'),
  ...
  ativar() {
    ...
    for (let i = 0; i < 4; i++) {
      if (loja.estado[i] === null) localStorage.setItem(`item${i}`, 'comprar');
      loja.estado[i] = localStorage.getItem(`item${i}`);

      loja.item[i].src = `assets/item${i}${loja.estado[i]}.png`;
...
loja.botaoSair.src = 'assets/loja_sair.png';

var powerup = {
  tipo: 0,
  img: new Image(),
  musica1: new Audio('sounds/powerup.mp3'),
  musica2: new Audio('sounds/powerup2.mp3'),
  musica3: new Audio('sounds/powerup3.mp3'),
  x: -44,
  y: -104,
  largura: 44,
  altura: 104,
...
powerup.img.src = 'assets/powerup.png';
...
var nuvem = {
  ...
  posicionar() {
    for (let i = 0; i <= this.num; i++) {
      ...
      if (this.tamanho < 0.35) {
        this.vel.push(Math.random() * 0.2 + 0.15);
  ...
  mover() {
    ...
    if (powerup.ativo && powerup.tipo === 1) nuvem.vel.fill(40);
  },
};
nuvem.img.src = 'assets/nuvem1.png';
...
botao.jogar.src = 'assets/botaojogar.png';
botao.sobre.src = 'assets/botaosobre.png';
botao.pause.src = 'assets/botaopause.png';
botao.minmax.src = 'assets/botaominmax.png';
botao.instru.src = 'assets/botaoinstru.png';
botao.musica.src = 'assets/botaomusica.png';
botao.sons.src = 'assets/botaosons.png';
...
function instrucoes() {
  ...
  jogo.onclick = function (event) {
    const xrato = event.pageX;
    const yrato = event.pageY;
...
function navegarMenuOver(event) {
  ...
  // BOTAO JOGAR
  if (clique(xrato, yrato, 0.33, 0.67, 0.51, 0.6)) {
    botaoSprite.jogar = 1;
    botaoSprite.instru = 0;
    botaoSprite.sobre = 0;
...
function moveJogador() {
  ...
  if (tecla_w && tecla_a === false && tecla_s === false && tecla_d === false) {
    ...
    (tecla_a && tecla_w === false && tecla_s === false && tecla_d === false) ||
    (tecla_a && tecla_w) ||
    (tecla_a && tecla_s)
  ) {
    jogador.spriteY = 2;
    ...
    (tecla_d && tecla_w === false && tecla_a === false && tecla_s === false) ||
    (tecla_d && tecla_w) ||
    (tecla_d && tecla_s)
  ) {
    jogador.spriteY = 1;
...
function animar() {
  ...
  if (delta > interval) {
    ...
    animacaoSprite(
      powerup.img,
      powerup.largura * powerup.spriteX,
      0,
      powerup.largura,
      powerup.altura,
      powerup.x,
    ...
    animacaoSprite(
      jogador.img,
      jogador.largura * jogador.spriteX,
      jogador.altura * jogador.spriteY,
      jogador.largura,
      jogador.altura,
      jogador.x,
      jogador.y,
    ...
    // AVISO DISPARO
    if (powerup.tipo === 2)
      c.drawImage(avisoDisparo, 0, 0, jogo.width, jogo.height);
...
function detectaColisao() {
  ...
  for (let i = 0; i < familiar.num; i++) {
    ...
    for (let j = 0; j <= lenhador.num; j++) {
      ...
      if (
        lenhador.x[j] < familiar.x[i] + familiar.largura &&
  ...
  for (let i = 0; i <= lenhador.num; i++) {
    if (
      jogador.x + sprite_x_removido < lenhador.x[i] + lenhador.largura &&
      ...
      n = 0;
      while (n !== 1) {
        const pos_x_temp = nAleatorio(jogo.width, lenhador.areax);
        const pos_y_temp = nAleatorio(124, 676);
        let sobreposicao = false;
    ...
    if (
      tiro.x < lenhador.x[i] + lenhador.largura &&
  ...
  for (let i = 0; i <= lenhador.num; i++) {
    if (jogador.andar === false) {
      ...
      if (
        jogador.x + sprite_x_removido + 200 <
          lenhador.x[i] + lenhador.largura &&
        ...
        if (movimento_vertical[i] === 0) {
          ...
          if (
            lenhador.y[i] + lenhador.altura / 2 <
  ...
  // STAG APANHA MOEDAS
  if (
    jogador.x + sprite_x_removido < moeda.x + moeda.largura &&
  ...
  if (
    jogador.x + sprite_x_removido < powerup.x + powerup.largura &&
    ...
    switch (powerup.tipo) {
      case 1:
        ...
        break;
      case 2:
        fundo.src = 'assets/jogoback2.png';
        zonaEntrada.src = 'assets/zona_entrada2.png';
        powerup.musica2.load();
        powerup.musica2.play();
        break;
      case 3:
        jogador.img.src = 'assets/stag2.png';
        jogador.largura *= 1.5;
        jogador.altura *= 1.5;
        sprite_x_removido *= 1.5;
        sprite_y_removido *= 1.5;
...
function processaBotoes(event) {
  ...
  if (clique(xrato, yrato, 0.93, 0.985, 0.9, 0.98)) {
    ...
    } else if (jogo.requestFullscreen) {
  ...
  if (clique(xrato, yrato, 0.082, 0.266, 0.557, 0.605)) {
    if (loja.estado[0] === 'comprar') {
      if (carteira > 50) {
        ...
        loja.item[0].src = `assets/item0${loja.estado[0]}.png`;
  ...
  if (clique(xrato, yrato, 0.305, 0.489, 0.557, 0.605)) {
    if (loja.estado[1] === 'comprar') {
      if (carteira > 100) {
        ...
        loja.item[1].src = `assets/item1${loja.estado[1]}.png`;
  ...
  if (clique(xrato, yrato, 0.52, 0.704, 0.557, 0.605)) {
    if (loja.estado[2] === 'comprar') {
      if (carteira > 250) {
        ...
        loja.item[2].src = `assets/item2${loja.estado[2]}.png`;
  ...
  if (clique(xrato, yrato, 0.734, 0.916, 0.557, 0.605)) {
    if (loja.estado[3] === 'comprar') {
      if (carteira > 500) {
        ...
        loja.item[3].src = `assets/item3${loja.estado[3]}.png`;
...
function processaBotoesFim(event) {
  ...
  if (clique(xrato, yrato, 0.355, 0.645, 0.705, 0.785)) {
    window.open(
      `https://twitter.com/intent/tweet?text=A%20minha%20pontuação%20no%20%40StagTheSavior%20foi%3A%20${pontos.totais}!%F0%9F%8E%89%20%23Games4Nature%20%23LabMM3%20%23Javascript`,
    );
  }
...
function clique(xrato, yrato, xmin, xmax, ymin, ymax) {
  if (
    xrato > tLargura * xmin + (innerWidth - tLargura) / 2 &&
